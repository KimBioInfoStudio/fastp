...
WriterThread::WriterThread(Options* opt, string filename, bool isSTDOUT){
    ...
    // ISA-L compression level (map fastp 1-9 -> ISA-L 0-3)
    mIsalLevel = 1;
    if(mOptions->compression <= 2) mIsalLevel = 0;
    else if(mOptions->compression <= 5) mIsalLevel = 1;
    else if(mOptions->compression <= 7) mIsalLevel = 2;
    else mIsalLevel = 3;
