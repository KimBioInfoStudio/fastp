...
// Parallel BGZF decompression with adaptive thread pool.
//
// Starts with 1 decompress thread. Reader monitors ring buffer fill level:
//   Ring < 25% full (consumer starved) → add a decompress thread (up to max)
//   Ring > 50% full (consumer can't keep up) → don't add
//
...
private:
    ...
    void decompWorker() {
        while (!mStop) {
            Slot* target = nullptr;
            {
                ...
                mDecompCv.wait_for(lk, std::chrono::milliseconds(1), [&]() {
                    return mStop || claimSlot(&target);
                });
    ...
    Slot* mSlots;
    int mRingSize;
    int mConsumeIdx, mConsumeOffset;
    int mProduceIdx;
    int mMaxPool;
